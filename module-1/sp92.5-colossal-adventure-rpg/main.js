...
const hero = new Character(youAreHere, 100, 15, playerInventory);
const enemy1 = new Character("a Goblin named Gobbie", 25, 10, [" bow", " axe"]);
...
while (diggingTunnels === true) {
    const doomTime = readlineSync.question("You enter a cold dark tunnel that smells of certain doom and your own fear. Do you wish to walk in [w], check your inventory [i], or run away [q]? ", {limit:["w", "i", "q"]})
    if (doomTime === "w") {
        let enemyCheck = getRandomNumber(1, 4);
        let whichOne = getRandomNumber(0, enemies.length - 1)
        if (enemyCheck === 2) {
            let whoGonnaKillYa = enemies[whichOne];
